...
import { brandsSlice, searchBrands, selectBrandsSlice, setBrandsSelection } from '../stores/brands/BrandStore';
import { deleteBrandByIds } from '../respositories/brands/BrandsRepo';
...
function Brands() {
	...
	const brand = useSelector(selectBrandsSlice);
	const onDeleteUser = () => {
		dispatch(deleteBrandByIds({ ids: brand.selectedRows }));
	};
	useEffect(()=>{
		dispatch(setBrandsSelection([]));
	},[])
	return (
		<Root
			content={
							...
							<div className="grow-0">
								<div className="flex  sm:flex-row items-start justify-between space-x-10 space-y-0">
									<TextField
										...
										placeholder="Search"
										value={brand.searchText}
										onChange={(e) => dispatch(searchBrands(e.target.value))}
										maxRows={7}
										minRows={7}
										sx={{
											background: 'white',
									...
									<EscButton
										...
										to={`/addbrand`}
										onClick={() => {
											dispatch(setBrandForUpate(false));
										}}
										startIcon={<AddIcon />}
									>
										Create
...
export default withSlices([brandsSlice])(memo(Brands));
