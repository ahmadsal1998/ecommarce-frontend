...
import { baseUrl, loginUrl } from 'src/app/main/ecommerce/respositories/urls';
...
const useJwtAuth = <User, SignInPayload, SignUpPayload>(
		...
		const attemptAutoLogin = async () => {
			...
			if (isTokenValid(accessToken)) {
				try {
					...
					const response = await axios.get(baseUrl + authConfig.getUserUrl, {
						headers: { Authorization: `Bearer ${accessToken}` }
					});
					const roles = [];
					// eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-return
					response.data.data.role.subroles.forEach((data) => {
						try {
							// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
							roles.push(...data.routes);
						} catch (e) {
							/* empty */
						}
					});
					...
					const userData = {
						...
						role: response.data.data.role_id ? 'admin' : null, // guest
						data: {
							displayName: response.data.data.var_name,
							photoURL: response.data.data.image,
							email: response.data.data.var_email,
							shortcuts: [],
							settings: {},
							roles
						}
	...
	const signIn = async (credentials) => {
		const response = axios.post(`${baseUrl}${loginUrl}`, {
			var_email: credentials.email,
			var_password: credentials.password
		});
		...
		response.then(
			async (res) => {
				...
				const userData = {
					...
					role: res.data.data.role_id ? 'admin' : null, // guest
					data: {
						displayName: res.data.data.var_name,
						photoURL: res.data.data.image,
						email: res.data.data.var_email,
						shortcuts: [],
						settings: {},
						roles
					}
