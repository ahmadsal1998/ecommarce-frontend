...
import { navbarCloseMobile, selectEscNavbar } from 'app/theme-layouts/shared-components/navbar/store/navbarSlice';
import NavbarStyle1Content from './NavbarStyle1Content';

const navbarWidth = 280;
const hiddenWidth =180;
...
const StyledNavBar = styled('div')<StyledNavBarProps>(({ theme, open, position }) => ({
	...
	...(!open && {
		...
		...(position === 'left' && {
			marginLeft: `-${hiddenWidth}px`
		}),
		...(position === 'right' && {
			marginRight: `-${hiddenWidth}px`
		})
	}),
	...(open && {
		transition: theme.transitions.create('margin', {
