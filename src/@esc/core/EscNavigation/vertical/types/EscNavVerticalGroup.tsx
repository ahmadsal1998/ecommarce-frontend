...
/**
 * EscNavVerticalGroup is a component used to render a group of navigation items in a vertical layout.
 */
function EscNavVerticalGroup(props: EscNavItemComponentProps) {
		...
		() => (
			<>
				<Root
					...
					className={clsx('es-list-subheader flex items-center  py-10', !item.url ? 'cursor-default' : '')}
					onClick={() => onItemClick && onItemClick(item)}
					sx={item.sx}
					{...itemProps}
				>
					<ListItemText
						className="es-list-subheader-text"
						sx={{
							margin: 0,
...
const NavVerticalGroup = EscNavVerticalGroup;
