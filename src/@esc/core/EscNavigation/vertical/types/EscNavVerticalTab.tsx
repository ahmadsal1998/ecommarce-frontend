...
export type EscNavVerticalTabProps = Omit<EscNavigationProps, 'navigation'> & EscNavItemComponentProps;

/**
 *  The `EscNavVerticalTab` component renders vertical navigation item with an adaptable
 *  layout to be used within the `EscNavigation`. It only supports the `type`s of 'item',
 *  'selection' and 'divider'
 * */
function EscNavVerticalTab(props: EscNavVerticalTabProps) {
			...
			<Root sx={item.sx}>
				<ListItemButton
					...
					className={clsx(
						...
						'esc-list-item flex flex-col items-center justify-center p-12'
					)}
					...
					{dense ? (
						...
						>
							<div className="relative flex h-32 min-h-32 w-32 items-center justify-center">
								{item.icon ? (
									<EscSvgIcon
										className={clsx('esc-list-item-icon', item.iconClass)}
										color="action"
									>
										{item.icon}
									</EscSvgIcon>
						...
						<>
							...
							<ListItemText
								className="esc-list-item-text w-full grow-0"
								primary={item.title}
								classes={{
									primary:
										'text-12 font-medium esc-list-item-text-primary truncate text-center truncate'
								}}
							/>
						</>
					)}
...
const NavVerticalTab = EscNavVerticalTab;
