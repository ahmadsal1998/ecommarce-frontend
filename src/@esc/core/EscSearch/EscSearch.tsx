...
import { EscFlatNavItemType } from '../EscNavigation/types/EscNavItemType';
...
const Root = styled('div')(({ theme }) => ({
	...
	'& .EscSearch-suggestionsContainerOpen': {
		position: 'absolute',
		zIndex: 1,
	...
	'& .EscSearch-suggestion': {
		display: 'block'
	},

	'& .EscSearch-suggestionsList': {
		margin: 0,
		padding: 0,
		listStyleType: 'none'
	},
			...
			{variant === 'basic' ? (
				...
				<>
					<TextField
						...
						InputProps={{
							...
							classes: {
								input: 'EscSearch-input py-0 px-16 h-40 md:h-48 ltr:pr-48 rtl:pl-48',
				...
				<TextField
					...
					InputProps={{
						...
						classes: {
							input: 'EscSearch-input py-0 px-16 h-48 md:h-64'
						}
					}}
					variant="standard"
					{...other}
...
function renderSuggestion(suggestion: EscFlatNavItemType, { query, isHighlighted }: RenderSuggestionParams) {
	...
	return (
		...
		>
			<ListItemIcon className="min-w-40">
				{suggestion.icon ? (
					<EscSvgIcon>{suggestion.icon}</EscSvgIcon>
				) : (
...
function getSuggestions(value: string, data: EscFlatNavItemType[]): EscFlatNavItemType[] {
...
function getSuggestionValue(suggestion: EscFlatNavItemType) {
...
type StateType = {
	searchText: string;
	search: boolean;
	navigation: EscFlatNavItemType[];
	suggestions: EscFlatNavItemType[];
...
type ActionType =
	...
	| { type: 'setNavigation'; data: EscFlatNavItemType[] }
 ...
 * EscSearch component
 */
function EscSearch(props: EscSearchProps) {
	const {
		navigation = [],
		className,
		variant = 'full',
		...
		trigger = (
			<IconButton
				...
				size="large"
			>
				<EscSvgIcon>heroicons-outline:search</EscSvgIcon>
			</IconButton>
		)
	} = props;
	...
	function handleSuggestionSelected(
		...
		{ suggestion }: { suggestion: EscFlatNavItemType }
	) {
				...
				>
					<Autosuggest
						...
						theme={{
							...
							suggestionsList: 'EscSearch-suggestionsList',
							suggestion: 'EscSearch-suggestion'
						}}
