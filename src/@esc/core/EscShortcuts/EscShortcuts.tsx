...
/**
 * The EscShortcuts component is responsible for rendering a list of shortcuts based on the navigation and shortcuts props.
 * It uses various MUI components to render the list items and search input.
 * The component is memoized to prevent unnecessary re-renders.
 */
function EscShortcuts(props: EscShortcutsProps) {
	...
	const [searchResults, setSearchResults] = useState<EscFlatNavItemType[]>([]);
	const [shortcutItems, setShortcutItems] = useState<EscFlatNavItemType[]>([]);

	useEffect(() => {
		const _shortcutItems = shortcuts
			? shortcuts.map((id) => _.find(navigation, { id }))
			: ([] as EscFlatNavItemType[]);
					...
					<div className={clsx('flex flex-1', variant === 'vertical' && 'flex-col')}>
								...
								_item && (
									...
									>
										...
										>
											...
											>
												{_item.icon ? (
													<EscSvgIcon>{_item.icon}</EscSvgIcon>
												) : (
						...
						>
							<IconButton
								...
								size="large"
							>
								<EscSvgIcon sx={{ color: amber[600] }}>heroicons-solid:star</EscSvgIcon>
							</IconButton>
						</Tooltip>
					</div>
				);
...
function ShortcutMenuItem(props: {
	shortcuts: EscShortcutsProps['shortcuts'];
	item: EscFlatNavItemType;
		...
		>
			<MenuItem key={item.id}>
				<ListItemIcon className="min-w-40">
					{item.icon ? (
						<EscSvgIcon>{item.icon}</EscSvgIcon>
					) : (
...
export default memo(EscShortcuts);
