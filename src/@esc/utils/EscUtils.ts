...
/**
 * The EscRouteItemType type is a custom type that extends the RouteObject type from react-router-dom.
 * It adds an optional auth property and an optional settings property.
 */
export type EscRouteItemType = RouteObject & {
	auth?: string[] | [];
	settings?: unknown;
};

/**
 * The EscRoutesType type is a custom type that is an array of EscRouteItemType objects.
 */
export type EscRoutesType = EscRouteItemType[];

/**
 * The EscRouteConfigType type is a custom type that defines the configuration for a set of routes.
 * It includes an optional routes property, an optional settings property, and an optional auth property.
 */
export type EscRouteConfigType = {
	routes: EscRoutesType;
	settings?: unknown;
	auth?: string[] | [];
};

/**
 * The EscRouteConfigsType type is a custom type that is an array of EscRouteConfigType objects.
 */
export type EscRouteConfigsType = EscRouteConfigType[] | [];
...
/**
 * The EscUtils class provides utility functions for the Esc project.
 */
class EscUtils {
	/**
	...
	/**
	 * The setRoutes function sets the routes for the Esc project.
	 */
	static setRoutes(
		config: EscRouteConfigType,
		defaultAuth: EscSettingsConfigType['defaultAuth'] = undefined
	): EscRouteItemType[] {
	...
	static generateRoutesFromConfigs(
		configs: EscRouteConfigsType,
		defaultAuth: EscSettingsConfigType['defaultAuth']
	) {
		let allRoutes: EscRouteItemType[] = [];
		configs.forEach((config: EscRouteConfigType) => {
